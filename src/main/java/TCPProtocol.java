...
/**
 * 该接口定义了通用NIOServ类与特定协议之间的接口， 它把与具体协议相关的处理各种I/O的操作分离了出来，
...
class EchoSelectorProtocol implements TCPProtocol {
	...
	public void read(SelectionKey key) throws IOException {
		...
		if (bytesRead == -1) {
			//clntChan.close();
			System.out.println("服务端收到客户端消息F："+new String(buf.array(),0,(int)bytesRead));
			System.out.println("服务端读取数据完毕 ！");
		} else if (bytesRead > 0) {
			System.out.println("服务端收到客户端消息："+new String(buf.array(),0,(int)bytesRead));
